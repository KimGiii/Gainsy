...
**1순위 — 보안 정책 (한 PR로)**
- [ ] H-5 JWT Access Token 24h → 1h (`SecurityConstants`, `application.yml`) + iOS refresh 흐름 검증
- [ ] Security headers 명시 — HSTS / X-Content-Type-Options / X-Frame-Options / Referrer-Policy (`SecurityConfig.headers(...)`)
- [ ] JWT 기본 시크릿 제거 — `application.yml:60` `${JWT_SECRET}`만 두고 미설정 시 시작 실패
