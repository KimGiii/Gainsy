...
- [ ] H-2 `@Modifying` 쿼리에 `@Transactional` 누락 (`FoodCatalogRepository`)
- [ ] H-5 JWT Access Token 24시간 만료 → 15분~1시간 + refresh 회전
- [ ] M-1~M-9 (JWT 시크릿 기본값, N+1, cron zone 명시, 스택 로깅, LIKE 이스케이프, prefix 하드코딩, security headers 등)
