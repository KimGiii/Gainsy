...
- 2026-05-21 기준 (최신), 백엔드 코드 리뷰(`docs/retrospectives/2026-05-21-backend-code-review.md`) 후 CRITICAL 2건·HIGH 4건을 dev 브랜치에 반영. 컨트롤러 JWT 직접 파싱 제거(`@CurrentUserId`), CORS 와일드카드 금지, RateLimitingFilter X-Forwarded-For 신뢰 제거, 페이지 크기 상한 도입, `GoalService.getGoalProgress` 조회/쓰기 분리(체크포인트 스케줄러 신설), `NotificationService` 대형 트랜잭션 분리. 컨트롤러·서비스 단위 테스트 전부 통과.
