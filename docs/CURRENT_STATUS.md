...
- 2026-05-04 기준, FCM 알림 연결(백엔드 FcmService + NotificationService + WeeklyNotificationScheduler + V14 migration, iOS FcmTokenUploader + 푸시 라우팅) 구현 완료로 Phase 5를 95%로 상향했다.
- 2026-05-04 기준, 진행 사진 삭제 API(백엔드) + iOS 삭제 UX + 비교 모드 구현 완료로 Phase 4를 100%로 상향했다.
- Phase 4 완성: 사진 삭제(소유권 검증 soft-delete + iOS context menu/상세 화면 삭제 버튼), 비교 모드(그리드 2장 선택 → 좌우 분할 `PhotoCompareView`, 날짜·체중 오버레이).
- 2026-05-04 기준, 식품 카탈로그 사용 횟수 추적(usage_count) + 사용자 직접 식품 등록(Custom Food) + 검색 공개화 + iOS 직접 등록 UI 구현 완료로 Phase 3을 100%로 상향했다.
- Phase 3 완성: 사용자가 누구나 공용 식품 DB에 직접 등록 가능(중복 제거, 정규화), 검색 시 usage_count DESC 정렬, 식단 기록 추가/삭제 시 사용 횟수 자동 카운팅, iOS 검색 없음 시 직접 등록 화면 제공.
...
- Phase 4는 진행 사진 iOS 16 호환 및 isBaseline 직렬화 버그 수정 반영으로 92%로 유지 중(EXIF 제거, 썸네일 생성 미완료).
- Phase 5는 Insights API(주간 회고·변화 분석) 풀스택 구현 + 테스트 완료 + iOS 연동까지 반영해 82%로 유지 중(FCM 알림 미완료).
...
- [ ] iOS ViewModel 테스트 보강 (`HomeViewModel`, `GoalProgressViewModel`, `ProgressPhotoViewModel`)
- [ ] 주요 도메인 권한 경계 통합 테스트
...
- 백엔드: 측정 CRUD + atOrBefore + 진행 사진 presigned URL MVP + 경로 정합성 + isBaseline 직렬화 버그 수정 + 사진 삭제 API(soft-delete, 소유권 검증) 완료
- iOS: 모든 둘레 입력 필드 + LatestStatsCard + 진행 사진 목록/상세/업로드 + iOS 16 호환 수정 + 삭제 UX(context menu + 상세화면 버튼) + 비교 모드(PhotoCompareView) 완료
- 보완 사항(출시 준비 단계): EXIF 제거, 썸네일 생성, 업로드 후처리 파이프라인
...
- 남은 것: iOS ViewModel 테스트 보강 (`HomeViewModel`, `GoalProgressViewModel`, `ProgressPhotoViewModel`)
...
3. **출시 준비** — `application-prod.yml` 점검, 환경 변수 체크리스트, 헬스체크/모니터링

### iOS 우선순위

1. **ViewModel 테스트 보강** — `HomeViewModel`, `GoalProgressViewModel`, `ProgressPhotoViewModel` (삭제·비교 포함)
2. 홈/마이/기록 화면 회귀 점검 및 스타일 일관성 보강
3. 접근성 점검 (Dynamic Type, VoiceOver 기본 대응)
4. 로딩/에러/빈 상태 화면 일관성 정리
5. TestFlight 배포 전 체크리스트 문서화
...
**마지막 업데이트**: 2026-05-04 (Phase 5 FCM 알림 연결 완료 → 95%)
