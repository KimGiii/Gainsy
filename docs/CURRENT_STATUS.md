...
- 2026-05-22 기준 (최신), 목표 진행 UX 개선 — 목표 생성 시 `startValue` 미입력 시 최신 신체 측정값으로 자동 보강, startDate에 "시작" 라벨 체크포인트 자동 생성으로 히스토리에 시작점이 항상 노출되도록 변경. iOS는 `CheckpointRow`에 깃발 아이콘 + "시작" 뱃지 표시, AddGoalView에 자동 채우기 안내 문구 추가.
