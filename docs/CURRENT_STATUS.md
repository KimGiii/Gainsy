...
Phase 5: 목표 & 인사이트        ████████████████████ 100%
Phase 6: MVP 출시 준비          ██████████████████░░  92%
```

## 현재 판단 기준

- 2026-05-15 기준 (최신), App Store Connect 메타데이터·스크린샷·앱 심사 정보·가격(무료)·Privacy Labels(추적=Yes, 광고 데이터 포함) 모두 입력·게시 완료. NSUserTrackingUsageDescription ↔ Privacy Labels 충돌 해소 완료. iPad 13" 스크린샷 업로드 → build 4 TestFlight 업로드 → 심사 제출이 남아 있어 Phase 6을 92%로 설정했다.
...
- [ ] **iPad 13" 스크린샷 업로드** ← 다음 단계 (2064×2752)
- [ ] **Xcode Archive build 4 → TestFlight 업로드** (광고 코드 복원 완료)
- [ ] **심사 제출** (build 4 선택 후 "심사를 위해 제출" 클릭)
- [ ] TestFlight 외부 테스터 5~10명 초대
...
### 즉시 처리 (Phase 6 마무리 — 나머지 8%)

1. **iPad 13" 스크린샷** — iPad Pro 13" 시뮬레이터(2064×2752) 캡처 후 App Store Connect 업로드
2. **Xcode Archive build 4 → TestFlight** — 광고 코드 복원된 상태로 Archive → App Store Connect 업로드
3. **심사 제출** — build 4 선택 → "심사를 위해 제출" 클릭
...
**마지막 업데이트**: 2026-05-15 (Phase 6 92% — App Store Connect 전체 메타데이터·Privacy Labels 완료, iPad 스크린샷·build 4·심사 제출 대기)
