...
- [ ] **Xcode Archive → TestFlight 업로드** ← 다음 단계
- [ ] **dev → prod PR merge** (B3~B6 커밋 prod 반영, 현재 502 원인)
- [ ] App Store Connect 스크린샷 (6.7" iPhone, 1290×2796, 3~10장)
- [ ] 앱 설명·키워드·부제 작성
- [ ] TestFlight 테스터 초대 (5~10명 smoke test)
- [ ] App Review 정보 입력 (데모 계정, 리뷰 노트)
- [ ] 심사 제출
...
1. **dev → prod PR merge** — 현재 prod 서버 502 원인. merge 후 `dev-to-prod` 워크플로우 실행 → Spring Boot 재배포
2. **Xcode Archive → TestFlight** — Product → Archive → Distribute → App Store Connect → Upload (수동)
3. **502 확인** — `curl -I https://api.gainsy.site/actuator/health` 200 응답 검증

### App Store Connect 준비

1. 스크린샷 6.7" iPhone (1290×2796) 3~10장
2. 앱 설명, 키워드, 부제 작성
3. TestFlight 테스터 5~10명 초대 → smoke test
4. App Review 정보 (데모 계정, 리뷰 노트)
5. 심사 제출
...
**마지막 업데이트**: 2026-05-15 (Phase 6 BLOCKER 완료 + 다크모드/Dynamic Type/접근성 + AWS 인프라 프로비저닝 완료)
