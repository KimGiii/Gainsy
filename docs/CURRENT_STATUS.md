...
1. **식품 카탈로그 관리 API** — 식품 승인/거절/숨김 기능 (사용자 직접 등록된 식품 QA)
...
2. **식품 직접 등록 검증** — 입력값 유효성 메시지, 중복 등록 안내, 네트워크 오류 처리
3. 진행 사진 비교/삭제 UX 추가
4. iOS ViewModel 테스트 보강 (`AddDietLogViewModel`, `HomeViewModel`, `GoalProgressViewModel`)
5. 홈/마이/기록 화면 회귀 점검 및 스타일 일관성 보강
