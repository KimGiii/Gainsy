...
관련 PR: [#33 dev→main 누적 배포](https://github.com/KimGiii/Gainsy/pull/33) · [#36 dev→prod 일괄 fix](https://github.com/KimGiii/Gainsy/pull/36)
...
1. **PR #36 머지 → prod 배포** — 목표·신체 측정·프로필 동기화 fix 일괄 적용. 머지 후 prod 컨테이너 자동 재기동 + canary 확인.
2. **iOS 새 빌드 App Store 업로드** — `55b717f` 에러 디코딩 fix가 사용자 폰에 도달하도록. build 번호 증가 + Archive → TestFlight → 심사 제출.
3. **사용자 회귀 확인** — 보고된 "목표 진행률 서버 오류" 재현 시도 → 422/500 안 나오는지 확인. 신체 측정 후 마이페이지 즉시 갱신 확인.

### 출시 후 (우선순위 순)

1. **APNs 실기기 테스트** — Firebase 콘솔에서 테스트 발송, WEEKLY_SUMMARY 탭 라우팅 확인 (M4)
2. **AWS Secrets Manager 이관** — 현재 GitHub Secrets → Secrets Manager (H2 잔여)
3. **백엔드 BACKEND_TODO 1순위** — H-2 `FoodCatalogRepository:66,71` `@Modifying` + `@Transactional`, N+1 배치 조회 전환
4. **User 엔티티 확장 검토** — `muscleMassKg` / `bodyFatPct` 보유 시 MUSCLE_GAIN / BODY_RECOMPOSITION 목표도 프로필 매핑 가능 (현재는 측정 fallback에만 의존)
5. **TestFlight 외부 테스터 5~10명 초대** (선택)
...
**마지막 업데이트**: 2026-05-23 야간 (목표↔신체 측정↔사용자 프로필 동기화 정합성 — PR #36)
