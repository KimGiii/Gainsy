...
Phase 0~5(인증/운동/식단/신체측정/진행사진/목표/인사이트) 모두 100% 완료. AWS 인프라 Terraform 프로비저닝 완료, CI/CD 파이프라인 완료, BLOCKER 6개 완료. TestFlight 업로드 → App Store Connect 메타데이터 → 심사 제출이 남아 있다.
...
### B6. App Store Connect Privacy Nutrition Labels ⚠️ 부분 완료
- [x] `PrivacyInfo.xcprivacy` — API 사용 이유 선언 완료
- [x] `Info.plist` 카메라/사진 라이브러리 권한 설명 완료
- [ ] **App Store Connect 양식 직접 입력** — Data Collected 항목 작성 필요
  - 신체측정/사진/식단/이메일 모두 명시
  - "Linked to User" / "Not Linked" 분류
  - 사용 목적: App Functionality (피트니스 트래킹)
  - Account Deletion 항목 체크 (이미 구현: `DELETE /api/v1/users/me`)
...
### 메타데이터
- [ ] 앱 이름: **Gainsy** (CFBundleDisplayName 반영 완료)
- [ ] 부제 (30자 이내)
- [ ] 키워드 (100자 이내)
- [ ] 설명 (4000자 이내)
- [ ] 프로모션 텍스트 (170자 이내)
- [ ] 카테고리: **건강 및 피트니스**
- [ ] 연령 등급: 4+ 또는 12+ (신체 데이터 입력 고려)
- [ ] 가격: 무료 (IAP 없음)

### 스크린샷
- [ ] **6.7" iPhone (필수)** — 1290×2796 또는 1320×2868, 3~10장
  - 홈 대시보드 / 운동 기록 / 식단 기록 / 신체 측정 / 진행 사진
- [ ] 6.5" iPhone (선택) — 1242×2688

### App Review 정보
- [ ] 데모 계정 (이메일/비밀번호)
- [ ] 리뷰 노트 (테스터 흐름 안내, AI 기능 사용법)
- [ ] 연락처

### 컴플라이언스
- [x] 수출 규정: `ITSAppUsesNonExemptEncryption: false` (`ios/project.yml`) 설정 완료
- [ ] IDFA 사용 안 함 확인 (Firebase Analytics 사용 시 SKAdNetwork 고려)
- [x] Account Deletion: `DELETE /api/v1/users/me` + iOS MyPage 구현 완료

### 출시 옵션
- [ ] 수동 출시(승인 후 수동 버튼) vs 자동 출시 결정
...
**즉시 (이번 주)**
1. dev → prod PR merge — 현재 prod 502 원인
2. Xcode Archive → TestFlight 업로드
3. 502 해소 확인 (`curl -I https://api.gainsy.site/actuator/health`)
4. B6 App Store Connect Privacy Nutrition Labels 양식 작성
5. TestFlight 테스터 5~10명 초대

**다음 주**
6. Smoke 시나리오 9개 통과 확인
7. App Store Connect 메타데이터 + 스크린샷 업로드
8. App Review 정보 입력
9. 심사 제출
