...
**최종 업데이트**: 2026-05-20
**현재 상태**: 재심사 거절 3건 대응 완료 · 재제출 + 영상 회신 준비 중

## 상황 요약

- 2026-05-15: 1차 심사 거절 (Guideline 2.1(a), 백엔드 502). → [대응 기록](APPSTORE_REVIEW_REJECTION_2026_05_15.md). 2026-05-18 502 해소 후 재제출 (build 6).
- 2026-05-19: 2차 심사 거절 3건 — Guideline **1.4.1**(의학 정보 출처 누락) · **2.5.1**(HealthKit 식별 누락) · **2.1**(ATT 프롬프트 미발견). → [대응 기록](APPSTORE_REVIEW_REJECTION_2026_05_19.md).
- 2026-05-20: 대응 PR [Gainsy#22](https://github.com/KimGiii/Gainsy/pull/22) 생성, 시뮬레이터 빌드 검증 완료. **TestFlight 업로드 + App Review 회신(ATT 영상 첨부) + 재제출**이 남은 즉시 작업.
...
- 개인정보 처리방침: `https://kimgiii.github.io/Gainsy/docs/legal/privacy.html`
- 이용약관: `https://kimgiii.github.io/Gainsy/docs/legal/terms.html`
- GitHub Pages(prod 브랜치 `docs/legal/`) 서빙 완료
- 앱 내 마이페이지 링크 노출: 확인 필요

### B7. 2026-05-19 재심사 거절 3건 대응 ⚠️ 코드 완료 · 재제출 대기
- [x] **1.4.1** 의학 정보 출처 — `MedicalSourcesView` 추가 (WHO·대한비만학회·식약처·USDA·면책 고지). BMI 입력/최근 측정/영양 카드/마이페이지 4곳에서 진입
- [x] **2.5.1** HealthKit 식별 — 미사용인 `NSHealth{Share,Update}UsageDescription` 키 제거
- [x] **2.1** ATT 프롬프트 — `TrackingPermissionView` 사전 설명 화면 도입, 스플래시 종료 후 호출
- [x] dev → prod PR [Gainsy#22](https://github.com/KimGiii/Gainsy/pull/22) 생성, 시뮬레이터 빌드 검증 완료
- [ ] PR merge → Xcode Archive build 7 → TestFlight 업로드
- [ ] **ATT 권한 요청 실기기 화면 녹화** (App Review Information > Notes 첨부 필수)
- [ ] App Store Connect 메시지에 회신하여 재심사 제출
...
- [x] HealthKit 미사용으로 결정 — `NSHealthShareUsageDescription`·`NSHealthUpdateUsageDescription` 키 제거 (2026-05-20, Guideline 2.5.1 재거절 대응)
- [ ] HealthKit 도입 시: plist 키 재추가 + 실제 코드 통합(`import HealthKit`) + UI 식별 라벨을 한 PR에서 동시 처리. 마케팅 활용 금지(App Review Guidelines 5.1.3) 준수 명시 필요
...
- [x] 데모 계정 입력 완료
- [x] 리뷰 노트 입력 완료
- [x] 지원 URL: `https://kimgiii.github.io/Gainsy/docs/legal/privacy.html`
- [x] 저작권: `© 2026 KimGiii`
...
8. [x] Xcode Archive build 4 → TestFlight 업로드 ✅ 완료 (2026-05-15)
9. [x] 1차 심사 제출 ✅ 완료 (2026-05-15) — 거절
10. [x] 재심사 제출 (build 6) ✅ 완료 (2026-05-18) — 거절
11. [x] **B7 거절 3건 코드 대응** ✅ 완료 (2026-05-20, [PR #22](https://github.com/KimGiii/Gainsy/pull/22))
12. [ ] **PR #22 머지** (dev → prod)
13. [ ] **Xcode Archive build 7 → TestFlight 업로드**
14. [ ] **실기기에서 ATT 프롬프트 화면 녹화** (신규 설치 → 스플래시 → 사전 설명 → 시스템 ATT 프롬프트 → 후속 화면)
15. [ ] **App Store Connect 회신**: 거절 메시지에 ① HealthKit 키 제거 사유 ② 의학 정보 출처 추가 위치 ③ ATT 영상 첨부 — 3건 회신
16. [ ] **build 7 재심사 제출**

**출시 후**
17. TestFlight 외부 테스터 5~10명 초대
18. H1 prod yml 강화, H6 Health Check
19. M1~M4 UX 보강
20. AWS Secrets Manager 이관 (선택)
...
| [docs/exec-plans/APPSTORE_REVIEW_REJECTION_2026_05_15.md](APPSTORE_REVIEW_REJECTION_2026_05_15.md) | 1차 거절(502) 대응 기록 |
| [docs/exec-plans/APPSTORE_REVIEW_REJECTION_2026_05_19.md](APPSTORE_REVIEW_REJECTION_2026_05_19.md) | 2차 거절(1.4.1·2.5.1·2.1) 대응 기록 |
