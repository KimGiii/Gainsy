...
### B6. App Store Connect Privacy Nutrition Labels ⚠️ 코드 완료 / 웹 양식 입력 필요
- [x] `PrivacyInfo.xcprivacy` — 전체 데이터 타입 선언 완료 (이메일, 닉네임, 건강, 운동, 사진, 식단, User ID, Device ID)
- [x] `Info.plist` 카메라/사진 라이브러리 권한 설명 완료
- [ ] **App Store Connect 웹 양식 직접 입력** — [APPSTORE_PRIVACY_LABELS.md](APPSTORE_PRIVACY_LABELS.md) 가이드 참고
